 ...
 */
function isSyntheticBackspaceEvent(event) {
  return (event === null || event === void 0 ? void 0 : event.type) === 'keydown' && (event === null || event === void 0 ? void 0 : event.key) === 'Backspace' && (event === null || event === void 0 ? void 0 : event.keyCode) === 8 && (event.isTrusted === false || !(event instanceof KeyboardEvent));
}
/**
 ...
 */
function installSyntheticBackspaceGuard(view) {
  ...
  view.someProp = function (propName, f) {
    if (propName !== 'handleKeyDown' || typeof f !== 'function') {
      return origSomeProp(propName, f);
    }
    return origSomeProp(propName, prop => {
      if (typeof prop !== 'function') {
        return f(prop);
      }
      return f((innerView, event) => {
        ...
        if (innerView.composing && innerView.state.selection.$from.parent.type.name === 'inputSlot' && isSyntheticBackspaceEvent(event)) {
          return false;
        }
 ...
 */
function removeSyntheticBackspaceGuard(view) {
  const patchedView = view;
    ...
    props: {
      handleKeyDown(view, event) {
        ...
        if (node.type.name === 'inputSlot') {
          ...
          const slotVisibleText = node.textContent.replace(new RegExp(_constants.strings.ZERO_WIDTH_CHAR, 'g'), '');
          if (slotVisibleText.length === 0 && node.textContent.length > 0 && (event.key === 'ArrowLeft' || event.key === 'ArrowRight')) {
          ...
          if (event.key === 'Backspace' && selection.empty) {
            const zeroWidthRegex = new RegExp(_constants.strings.ZERO_WIDTH_CHAR, 'g');
            ...
            if ($from.pos === $from.end() && visibleText.length === 1) {
              const tr = state.tr.insertText(_constants.strings.ZERO_WIDTH_CHAR, $from.start(), $from.end());
              dispatch(tr);
              event.preventDefault();
              return true;
            }
