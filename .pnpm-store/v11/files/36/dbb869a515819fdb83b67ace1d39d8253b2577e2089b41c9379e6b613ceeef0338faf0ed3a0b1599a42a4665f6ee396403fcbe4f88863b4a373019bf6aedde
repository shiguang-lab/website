...
function withField(Component, opts) {
  let SemiField = (props, ref) => {
    ...
    if (keepState && inArrayField) {
      warning(true, `[Semi Form]: 'keepState' is not supported on Field "${field}" inside <ArrayField/>. It will be ignored. Use add/remove on the ArrayField to manage array items instead.`);
      keepState = false;
    }
