  ...
  const node = (0, _react.useMemo)(() => {
    if (status === MESSAGE_STATUS.LOADING) {
      ...
      if (typeof content === 'string') {
        const rawText = shouldEscapeHtml ? (0, _escapeHtml.escapeHtmlInMarkdown)(content) : content;
        ...
        realContent = content.map((item, index) => {
          var _a;
          if (item.type === 'text') {
            const rawText = shouldEscapeHtml ? (0, _escapeHtml.escapeHtmlInMarkdown)(item.text) : item.text;
